                  ...
                  `String' d_name)
{
	...
	converged = inner_irls(mu, eta, check_separation, data=., z=., deviance=., eps=., separated_obs=J(0, 1, .), lambda)
	...
	if (N_sep) {
		...
		backup_iter = iter
		converged = inner_irls(mu, eta, 0, data=., z=., deviance=., eps=., separated_obs=J(0, 1, .), lambda)
